"""Eksperimen fitur kalender tambahan: weekend/day-of-week + libur nasional + fase pandemi.
Pipeline identik experiment_zi_geocoded.py (16 region, anti-leakage, log1p train-only).
Config: Hybrid base (128/128, do=0, lr=1e-3, bs=64) x 3 seeds (42, 7, 123).
Varian:
  V1: + weekend biner + holiday + fase (F=6)
  V2: + day-of-week one-hot + holiday + fase (F=12)
Bandingkan dengan baseline (F=2, R2 0.0523) dari experiment_results_zi_geocoded.json.
Output: skrip/exp_features.json + log.
"""
import json
import numpy as np
import pandas as pd
...
OUT = r"E:\Download\Jurnal\results\exp_features.json"
...
# ---------------------------------------------------------------------------
def build_calendar_features(t_all, variant):
    """Kembalikan array (n_samples, n_extra) fitur hari target."""
    feats = []
    for m in t_all:
        t = m.start_time
        dow = t.dayofweek
        ...
        # Fase pandemi: refocusing (Jul-Des 2020), gelombang (2021-2022), endemi (2023+)
        if t <= pd.Timestamp("2020-12-31"):
            ...
            fase = [0.0, 0.0]
        if variant == "V1":
            feats.append([is_weekend, is_hol] + fase)
        else:  # V2: dow one-hot
...
def add_calendar(X_np, cal_feats):
    """Tile fitur kalender ke semua region dan semua timestep window.
    X_np: (B, N, T, 2). Return (B, N, T, 2 + n_extra)."""
    B, Nn, T, F = X_np.shape
    n_extra = cal_feats.shape[1]
    out = np.zeros((B, Nn, T, F + n_extra), dtype=np.float32)
    out[:, :, :, :F] = X_np
    # cal_feats[b] shape (n_extra,) -> tile ke (Nn, T, n_extra)
    for b in range(B):
...
def main():
    ...
    cal = build_calendar_features(t_all, "V1")
    Xtr_v1 = add_calendar(Xtr_n, cal[tr_mask])
    Xva_v1 = add_calendar(Xva_n, cal[va_mask])
    Xte_v1 = add_calendar(Xte_n, cal[te_mask])

    cal2 = build_calendar_features(t_all, "V2")
    Xtr_v2 = add_calendar(Xtr_n, cal2[tr_mask])
    Xva_v2 = add_calendar(Xva_n, cal2[va_mask])
    Xte_v2 = add_calendar(Xte_n, cal2[te_mask])

    results = {}
    for vname, (Xtr_f, Xva_f, Xte_f) in {
        "V1_weekend_holiday_fase": (Xtr_v1, Xva_v1, Xte_v1),
        "V2_dow_holiday_fase": (Xtr_v2, Xva_v2, Xte_v2),
    }.items():
        print("=== {} ===".format(vname), flush=True)
        preds, p_clfs = [], []
        per_seed = []
        for s in SEEDS:
    ...
    out = {
        "config": CFG,
        "seeds": SEEDS,
        "baseline": {"R2": float(np.mean(res["models"]["Hybrid"]["R2"])),
                     "RMSE": float(np.mean(res["models"]["Hybrid"]["RMSE"])),
                     "palembang_rmse": float(np.sqrt(np.mean(
                         (yte[:, REGIONS.index("Palembang")] - pred_base[:, REGIONS.index("Palembang")]) ** 2)))},
        "results": results,
    }
