# -*- coding: utf-8 -*-
"""Batas atas zero-inflated + uji model zero-inflated harian."""
import numpy as np
import pandas as pd
...
# ---- Batas atas: klasifikasi sempurna + train-mean amount ----
nonzero_tr = np.where(ytr > 0, ytr, np.nan)
train_mean_nz = np.nanmean(nonzero_tr, axis=0)  # mean amount per region (nonzero days)
train_mean_nz = np.nan_to_num(train_mean_nz)
perfect = np.where(yte > 0, train_mean_nz, 0.0)
print(f"\nBatas atas (klasifikasi sempurna + train-mean amount): R2={r2_score(yte, perfect):.4f}")
...
# Normalisasi X: log1p pooled (untuk input)
X_all_flat = np.log1p(X_all.reshape(-1, N))
mu = X_all_flat.mean(); sd = X_all_flat.std()
X_all_n = (np.log1p(X_all.reshape(-1, N)) - mu) / sd
X_all_n = X_all_n.reshape(X_all.shape)
Xtr_n, Xte_n = X_all_n[tr_mask], X_all_n[te_mask]

# 1) Klasifikasi ada/tidak penjualan
...
# 2) Regresi jumlah (log1p) hanya pada sampel non-zero
mask_tr = ytr > 0
Xamt_tr = Xtr_n[mask_tr]
yamt_tr = np.log1p(ytr[mask_tr])
amt = LSTMAmount()
train_reg(amt, Xamt_tr, yamt_tr)
p_amt = np.expm1(predict(amt, Xte_n))
p_amt = np.clip(p_amt, 0, None)

# 3) Gabungan: P(ada) * jumlah
p_zi = p_clf * p_amt
print(f"Zero-inflated: R2={r2_score(yte, p_zi):.4f} RMSE={np.sqrt(np.mean((yte - p_zi)**2)):,.0f} "
