# -*- coding: utf-8 -*-
"""
Eksperimen final v2 (GEOCODED): Spatiotemporal Prediction of Medical Device
Sales Using Hybrid LSTM-GNN in South Sumatra - protokol HARIAN ZERO-INFLATED
dengan fitur eksogen COVID-19.

Perbedaan vs experiment_zi_final.py: koordinat region TIDAK lagi mean mentah
dari kolom latitude/longitude data (level kota), melainkan centroid tertimbang
faktur dari koordinat AKTUAL pelanggan hasil geocoding
(skrip/customers_geocoded_final.csv, 118/156 pelanggan = 98,8% faktur).

- Sumber koordinat: PHOTON (90), SIRS Kemenkes (21), idalamat (3),
  lewatmana (2), Google Maps (1), Medicastore (1)
- Centroid region = mean (lat, lon) tertimbang jumlah faktur dari pelanggan
  FOUND_GEO + FOUND_SIRS
- Musi Rawas Utara DIGABUNG ke Musi Rawas (keputusan 19 Agu 2026): data MRU
  hanya 2 faktur dalam 5 tahun (fraksi nol 99,9%); MRU dimekarkan dari Musi
  Rawas pada 2013 -> 16 region
- Threshold graf dihitung ulang dengan aturan yang sama seperti eksperimen
  asli: threshold = jarak 3-NN maksimum sehingga 0 kandidat edge terpotong
- Semua hal lain identik (split, window, model, seeds, ablasi, metrik)

Data: view_penjualan_detail data hingga oktober.xlsx
- Target: jual_total (nilai faktur) per kab/kota per HARI
- 16 kab/kota Sumsel (pemetaan dari 34 kota mentah; Musi Rawas Utara -> Musi Rawas)
...
- Graf: Haversine, k=3, threshold dihitung ulang, undirected, tanpa self-loop
- Model zero-inflated dua tahap: klasifikasi (BCE) + regresi (masked MSE, log1p)
...
# ---------------------------------------------------------------------------
# 1b. Koordinat region dari geocoding aktual (bukan mean mentah data)
# ---------------------------------------------------------------------------
geo = pd.read_csv(GEO)
geo = geo[geo["status"].isin(["FOUND_GEO", "FOUND_SIRS"])].copy()
# Jumlah faktur per pelanggan dihitung dari data penjualan itu sendiri
...
print(f"Geocoded: {len(geo)} pelanggan FOUND, faktur tercakup "
      f"{geo['n_inv_data'].sum():.0f} dari {len(df)} ({geo['n_inv_data'].sum() / len(df):.1%})",
      flush=True)

# Centroid tertimbang faktur per region
...
# Fallback: tidak ada region tanpa pelanggan ter-geocode setelah Musi Rawas
# Utara digabung ke Musi Rawas (keputusan 19 Agu 2026: data MRU hanya 2 faktur)
FALLBACK = {}
for r, (lat, lon) in FALLBACK.items():
    if np.isnan(coords.loc[r, "lat"]):
        coords.loc[r, "lat"], coords.loc[r, "lon"] = lat, lon
        print(f"Fallback {r}: pusat region ({lat}, {lon})", flush=True)

# Bandingkan dengan koordinat lama (mean mentah data)
coords_old = df.groupby("region").agg(lat=("Latitude", "mean"), lon=("longitude", "mean"))
cmp = pd.DataFrame({
    "lat_old": coords_old["lat"], "lon_old": coords_old["lon"],
    "lat_new": coords["lat"], "lon_new": coords["lon"],
})
print("\nKoordinat region: lama (mean data) vs baru (centroid geocoded):", flush=True)
...
n_days = len(panel)
print(f"\nPanel: {n_days} hari ({days[0]} s.d. {days[-1]}), {N} region", flush=True)
...
# Fitur COVID-19: indikator biner periode pandemi
...
# PENTING: mu/sd dihitung HANYA pada window training (anti-leakage, sesuai
# komentar reviewer poin 3: transformasi hanya dilatih menggunakan training set).
mu = np.log1p(Xtr[:, :, :, 0]).mean(); sd = np.log1p(Xtr[:, :, :, 0]).std()
print(f"Normalisasi log1p: mu {mu:.4f} sd {sd:.4f} (dihitung pada training saja)", flush=True)
X_all_n = X_all.copy()
...
X_all_1 = X_all[:, :, :, :1]
Xtr_1 = X_all_1[tr_mask]
mu1 = np.log1p(Xtr_1[:, :, :, 0]).mean(); sd1 = np.log1p(Xtr_1[:, :, :, 0]).std()
...
# ---------------------------------------------------------------------------
# 3. Graf: Haversine, k=3, threshold = max jarak 3-NN (0 kandidat edge terpotong)
# ---------------------------------------------------------------------------
...
THRESHOLD = float(knn_dists.max())  # aturan sama: tidak memotong kandidat edge
print(f"\nJarak 3-NN (koordinat baru): min {knn_dists.min():.2f}, "
      f"median {np.median(knn_dists):.2f}, max {knn_dists.max():.2f} km", flush=True)
print(f"Threshold dihitung ulang: {THRESHOLD:.2f} km (0 dari {N * K} kandidat edge terpotong)", flush=True)
...
partial = {
    ...
    "graph": {"n_edges": n_edges, "degrees": degrees.tolist(),
              ...
              "coords_old": coords_old.to_dict(),
              "geocode": {"n_customers_found": int(len(geo)),
                          "n_inv_found": int(geo["n_inv_data"].sum()),
                          "n_inv_total": int(len(df)),
                          "fallback": FALLBACK}},
...
# Varian 6: graf jaringan distribusi - semua rute berpusat di Palembang
# (pusat distribusi: Komplek Villa Kenten Blok G No. 2, Palembang) ->
# star graph: Palembang terhubung ke semua 16 region lain
PALEMBANG_IDX = REGIONS.index("Palembang")
star_adj = np.zeros((N, N))
star_adj[PALEMBANG_IDX, :] = 1
star_adj[:, PALEMBANG_IDX] = 1
np.fill_diagonal(star_adj, 0)
ablation["Distribution network (star from Palembang)"] = run_ablation(star_adj, "Distribution star")
